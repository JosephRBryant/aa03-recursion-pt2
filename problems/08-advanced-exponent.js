...
be recursively calling `advancedExponent`.

1.Find a way to visually see how many
times `advancedExponent` is being recursively called.

2.Write a more advanced version of the recursive `exponent` function that
you just wrote.

3.Instead of multiplying the base number by itself n power of
times, like you did previously, you will be

4.squaring the results of the base
number raised to the power of half of n power.

The following is math, not JavaScript:

exponent(b, 0) // 1 => base case
exponent(b, 1) // b => base case
exponent(b, n) // exponent(b, n / 2) ** 2             [for even n]
exponent(b, n) // b * (exponent(b, (n - 1) / 2) ** 2) [for odd n]
...
function advancedExponent(b, n) {
  ...
  // if (b * (advancedExponent(b, (n - 1) / 2) ** 2)){
  //   return "odd"
  // }
}
